<template>
  <AdminLayout>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="vendor in filteredVendors" :key="vendor.id" class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4">
                    ...
                    <div class="text-sm text-gray-500">{{ vendor.business_links?.[0]?.business_type || 'Not set' }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm text-gray-900">{{ vendor.phone_number || 'N/A' }}</div>
                    <div class="text-sm text-gray-500">{{ vendor.business_links?.[0]?.business_link || 'No link' }}</div>
                  </td>
                  <td class="px-6 py-4 whitespace-nowrap">
    ...
    <div v-if="vendorToDelete" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
        <h3 class="text-lg font-semibold text-gray-900 mb-2">Delete Vendor</h3>
        <p class="text-gray-600 mb-6">
          Are you sure you want to delete <strong>{{ vendorToDelete.business_name || vendorToDelete.first_name + ' ' + vendorToDelete.last_name }}</strong>?
