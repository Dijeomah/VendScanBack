          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
              <h1 class="text-2xl md:text-3xl font-bold text-gray-900">
                {{ vendor.business_links?.[0]?.business_data?.business_name || 'Our Menu' }}
              </h1>
              <div v-if="businessLink" class="flex items-center gap-4 mt-2 text-sm text-gray-600">
                <span v-if="businessLink.business_data?.business_type" class="flex items-center gap-1">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                  </svg>
                  {{ businessLink.business_data.business_type }}
                </span>
                <span v-if="businessLink.business_data?.phone_number" class="flex items-center gap-1">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                  </svg>
                  {{ businessLink.business_data.phone_number }}
                </span>
              </div>
            </div>
          </div>
